...
//set line width
c.lineWidth = 2 * canvas.width / 100;
...
for (var colors = 0; colors < 8; colors++) {
  //the x y and color is in the list
  clist.push([0, 0, '#ffe0e0']);
}
...
//six objects with colours
var maindraw = new DrawingObject();
...
var maincount = 0;
var divisor = 1;
var xchangemult = 0.05 + Math.random() / 5;
var ychangemult = 0.015 + Math.random()*1;
var oldy = 0;
...
function main() {
  ...
  xchange = Math.sin(maincount * xchangemult) * cwidth / 80
  //console.log(xchange)
  ychange = 1*cheight/900 + Math.cos(maincount * ychangemult) * cheight / 100
  ...
  for (var colors = 0; colors < 8; colors++) {
    ...
    //wjere the line was
    oldx = clist[colors][0] + colors*10* cwidth / 80
    oldy = clist[colors][1]

    //where the lines going
    newx = oldx + xchange
    newy = oldy + ychange

    //add the line to the maindraw object
    _DrawLine(oldx, oldy, newx, newy, clist[Math.floor(Math.random()*6)][2]);


    //now set the colors oldx values to what the new ones are
    clist[colors][0] += xchange
    clist[colors][1] += ychange
