...
//set line width
c.lineWidth = 2 * canvas.height / 20;
...
function WhenMouseMoved() {
  var colouring = Math.random() * 1000

  curcolour = randomcolor()

  if (Math.sqrt(Math.pow(mouseX - oldmx, 2) + Math.pow(mouseY - oldmy, 2)) > 200) {
    _DrawLine(mouseX, mouseY, oldmx, oldmy, curcolour)
    oldmx = mouseX
    oldmy = mouseY
  }
}
...
function randomcolor() {
  ...
  var colour = '#'
  for (var i = 0; i < 6; i++) {
    var letters = Math.random() * 3
    csegment = segments[Math.floor(letters)]
    colour += csegment
  }
  return (colour)
}

function randomlcolor() {
  var segments = ['0', '1', '2', '3', '4', '5']

  var colour = '#'
  for (var i = 0; i < 6; i++) {
    var letters = Math.random() * 6
    csegment = segments[Math.floor(letters)]
    colour += csegment
  }
  return (colour)
...
//drawing the starting diagonal squiggles
for (var linef = 0; linef < 400; linef++) {
  _DrawLine(linef * 20, -30, -30, linef * 20, randomlcolor())
}
